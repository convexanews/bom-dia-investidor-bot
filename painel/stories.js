...
function importSeed(){
  ...
  if(seed?.item){localStorage.removeItem('bdi-story-seed');const news=seed.item,source=clean(news.fonte||'Fonte original',60),articleBlock=Array.isArray(news.blocos)?news.blocos.find(Boolean):'';if(seed.project?.slides?.[0]){const slide=seed.project.slides[0];state={...defaults(),category:slide.category,headline:slide.headline,body:slide.body,source:String(slide.source||source).replace(/^Fonte:\s*/i,''),credit:slide.imageCredit||`Imagem editorial: ${source}`,cta:slide.cta,image:slide.image,imageIndex:0,originNews:seed.project.originNews,caption:seed.project.caption||`Fonte: ${source}`};return}state={...defaults(),category:clean((news.pilares?.[0]||'Mercado agora').replace(/^./,x=>x.toUpperCase()),28),headline:clean(editorial(news.titulo),130),body:storySummary(articleBlock||news.descricao||'Consulte a matéria original para entender o contexto completo.'),source,credit:`Imagem editorial: ${source}`,cta:'Confira no perfil →',image:articleImage(news.link,0),imageIndex:0,originNews:{title:editorial(news.titulo),link:news.link,source},caption:`Fonte: ${source}\n${news.link||''}`};return}
